...
| `io.FontDefault` | `ImGuiIO::FontDefault` | Same idea — the font used when nothing is pushed — but takes any LÖVE `Font` object rather than an `ImFont*` from the `io.Fonts` atlas. There is no glyph-range/font-merging machinery (`AddFontFromFileTTF`, `MergeMode`): LÖVE's own `Font:setFallbacks()` covers the "base font + symbol font" case natively, and the result is just assigned here. |
...
| `SetNextWindowSnap(side, cond)` / `GetWindowSnap()` | — (no equivalent) | imlove-only: pin a window to the left/right screen edge as a full-height side panel, or read which edge it's pinned to. Users can also snap/unsnap by dragging the title bar to/away from a screen edge. This is edge *snapping*, not ImGui's docking branch — no dock spaces, tabs, or splitters (a deliberate non-goal, see ROADMAP.md). See [api.md](api.md#snapping). |
...
- **`PushStyleVar` shape rules** — a plain number for a scalar field, a
  ...
  | `"snapZone"` | number | *(none)* | No ImGui equivalent (window snapping is imlove-only — see `SetNextWindowSnap` above): the mouse distance from a screen edge that counts as "on it" when dragging a title bar to snap/unsnap. |
